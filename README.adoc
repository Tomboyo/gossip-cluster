...
----
podman build --tag gossip .
podman run --het=host -e RELEASE_NAME=b gossip
----
...
If you do not want to fiddle with podman (or docker) and have elixir installed,
you can run this on both computers (setting the value of `--sname` differently
for each computer):
